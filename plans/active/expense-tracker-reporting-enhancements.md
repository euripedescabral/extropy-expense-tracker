...
Objective: expand the ledger into a stronger demo surface with dashboard summary metrics, a detailed report page, category budgets, CSV export, and spending trends visualization.
...
Authentication: budget routes require the same JWT bearer token as expenses and categories.

Authorization: budget records are scoped by authenticated `userId`; DynamoDB keys use the existing per-user partition strategy.

Input validation: budget amount/category validation is shared through `@expense-tracker/core`.
...
- Reporting math stays in pure functions so API, UI, and tests share behavior.
...
| API budgets | Main agent | `apps/api/src/lambda*`, `apps/api/src/repositories/dynamo*` | `pnpm test` |
